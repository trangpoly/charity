<x-app-layout>
    ...
    <div class="flex max-w-8xl mx-auto mt-16 space-x-8 mb-10">
        <div class="flex w-8/12 bg-white rounded-lg border border-gray-700">
            <div class="w-full m-4">
                <form id="form-editPost" action="{{ route('web.client.duplicate.store', ['product' => $post->id]) }}" method="POST"
                    ...
                    <div class="mb-5">
                        ...
                        <div class="box-avatar">
                            ...
                            <input type="hidden" name="avatar_old" value={{ $post->avatar }}>
                        </div>
                    </div>
                    <div class="mb-5">
                        <label for="message" class="mb-3 block text-base font-medium text-[#07074D]">
                        ...
                        <div class="flex m-4 box-images">
                            @foreach ($post->images as $item)
                                <div class="relative">
                                    ...
                                    <a href="" data-product-image-name="{{ $item->path }}"
                        ...
                        @if (session('limitImgMsg'))
                            <p class="ml-2 text-red-600 text-md mt-3">{{ session()->pull('limitImgMsg') }}</p>
                        @endif
    ...
    <script type="text/javascript">
        ...
        function previewImg() {
                ...
                $(".box-images").append(`
                    <div class="relative ">
                        <img class="image_add img_preview mr-2 w-[120px] h-[80px] rounded-lg shadow-xl"
                            src="${URL.createObjectURL(arrImgAdd[i])}" alt="">
                        <a href="" data-key = ${i}
                            class="absolute w-5 top-1 right-3 rounded-full bg-red-600 product-imageNew-delete">
                            <svg className="w-6 h-6" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                                    d="M6 18L18 6M6 6l12 12" />
            ...
            $(".product-imageNew-delete").on("click", function(e) {
                e.preventDefault();
        ...
        $(document).ready(function() {
            $('.product-image-delete').click(function(e) {
                ...
                var product_image_name = $(this).data('product-image-name');
                $(this).parent().css('display', 'none');
                $(this).parent().append(`
                        <input
                            class="block w-full text-sm text-gray-400 bg-white rounded border border-gray-300 cursor-pointer focus:outline-none "
                            name="images_hidden[]" id="multiple_files"
                            value="` + product_image_name + `">
                `);
            });
