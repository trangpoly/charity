...
@section('content')
    <div class="container">
        <div class="bg-blue-700 px-5 py-2">
            <p class="text-white font-semibold">Account Create</p>
        </div>
        <form action="{{ route('web.admin.account.saveCreate') }}" method="POST" class="w-8/12 ml-24 mt-5"
            enctype="multipart/form-data">
            @csrf
            <div class="flex">
                <p class="text-black w-3/12">Account Name<span class="text-red-700 ml-2">*</span></p>
                <div class="flex-col w-9/12">
                    <input type="text" name="account" value="{{old('account') ?? ""}}" id="" class="w-full border border-gray-500">
            ...
            <div class="flex mt-5">
                ...
                <div class="flex-col w-9/12">
                    <input type="email" name="email" value="{{old('email') ?? ""}}" id="" class="w-full border border-gray-500">
                    @if ($errors->has('email'))
            ...
            <div class="flex mt-5">
                ...
                <div class="flex-col w-9/12">
                    <input type="password" name="password" value="{{old('password') ?? ""}}" id="" class="w-full border border-gray-500">
