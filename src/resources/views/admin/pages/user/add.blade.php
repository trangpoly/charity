...
@section('content')
    <div class="container">
        <div class="bg-blue-700 px-5 py-2">
            <p class="text-white font-semibold">Create User</p>
        </div>
        <form action="{{ route('web.admin.user.store') }}" method="POST" class="w-8/12 ml-24 mt-5"
            enctype="multipart/form-data">
            @csrf          
            <div class="flex">
                <p class="text-black w-3/12">User Name<span class="text-red-700 ml-2">*</span></p>
                <div class="flex flex-col w-9/12">
                    <input type="text" name="name" id="" value="{{old('name')}}" class="w-full border border-gray-500">
                    @error('name')
            ...
            <div class="flex mt-5">
                ...
                <div class="flex flex-col w-9/12">
                    <input type="text" name="phone_number" id="" value="{{old('phone_number')}}" class="w-full border border-gray-500">
                    @error('phone_number')
                        <p class="text-red-500">{{$message}}</p>
                    @enderror
                </div>
            ...
            <div class="flex mt-5">
                ...
                <div class="flex flex-col w-9/12">
                    <input type="text" name="email" id="" value="{{old('email')}}" class="w-full border border-gray-500">
                    @error('email')
                        <p class="text-red-500">{{$message}}</p>
                    @enderror
                </div>
            ...
            <div class="flex mt-5">
                ...
                <div class="flex flex-col w-9/12">
                    <div class="w-9/12 flex space-x-8 text-gray-700">
                        <div class="space-x-2">
                            <input type="radio" checked id="1" name="status" value="1">
                            <label for="1">Active</label><br>
                        </div>
                        <div class="space-x-2">
                            <input type="radio" id="2" name="status" value="0">
            ...
            <div class="flex mt-5">
                ...
                <input type="text" name="facebook_id" id="" value="{{old('facebook_id')}}" class="w-9/12 border border-gray-500">
            </div>
            <div class="flex mt-5">
                <p class="text-black w-3/12">Google ID<span class="text-red-700 ml-2"></span></p>
                <input type="text" name="google_id" id="" value="{{old('google_id')}}" class="w-9/12 border border-gray-500">
            </div>
